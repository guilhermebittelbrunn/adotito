...
export default function LastRequests() {
  const columns = [
      ...
      render: (animal) => (
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500">{animal.specie.name}, {animal.age} {animal.age > 1 ? "anos" : "ano"}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (desc) => (
        <span className="truncate block max-w-xs" title={desc}>{desc}</span>
      ),
    },
    {
      title: "",
      ...
      render: (_, record) => (
        <Link href={`/associacao/requests/${record.id}`}>
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded bg-primary-100 text-primary-700 hover:bg-primary-200 text-xs font-medium transition-colors">
            Visualizar
            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="inline-block"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
          </span>
        </Link>
      ),
    },
  ...
  return(
      <div className="col-span-12 space-y-5 sm:space-y-6">
        <ComponentCard title="Últimas Requisições de Adoção" subTitle={
          <Link href="/associacao/requests">
            Ver todas
          </Link>
        }>
