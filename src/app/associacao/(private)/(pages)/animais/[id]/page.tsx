...
import { AnimalGenderEnum, AnimalSizeEnum, AnimalStatusEnum, IAnimal } from "@/types";
import { Card, Row, Col, Descriptions, Tag, Typography, Space } from "antd";
import Image from "next/image";
import { useParams } from "next/navigation";

const mockAnimalData: { data: IAnimal } = {
    data: {
        ...
        age: 1,
        weight: 4,
        status: AnimalStatusEnum.IN_ADOPTION,
                        ...
                        <Space direction="vertical" size="large" className="w-full">
                            <div>
                                ...
                                <Space>
                                    ...
                                    <Tag color={animal.status === AnimalStatusEnum.IN_ADOPTION ? 'green' : 'orange'}>
                                        {animal.status === AnimalStatusEnum.IN_ADOPTION ? 'Disponível para adoção' : 'Em processo de adoção'}
                                    </Tag>
                                </Space>
                            </div>
                            ...
                            <Descriptions column={1}>
                                ...
                                <Descriptions.Item label="Gênero">
                                    {animal.gender === AnimalGenderEnum.MALE ? 'Macho' : 'Fêmea'}
                                </Descriptions.Item>
                                <Descriptions.Item label="Porte">
                                    {animal.size === AnimalSizeEnum.SMALL ? 'Pequeno' :
                                     animal.size === AnimalSizeEnum.MEDIUM ? 'Médio' : 'Grande'}
                                </Descriptions.Item>
                            </Descriptions>
